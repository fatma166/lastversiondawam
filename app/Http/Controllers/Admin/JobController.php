...
class JobController  extends BaseController
{
    ...
    public function index($subdomain)
    {
     ...
     //add evalution_elements with staus 1 
        $evaluation= evaluation_element::where('company_id', $company_check->id)->count();
       // return $evaluation;
        
         if($evaluation==0)
         {
            $eval_array=['الحضور','الإلتزام','الزيارات','المهام'];
            foreach ($eval_array as $key => $value) 
            {
               $input=array('title'=>$value,'company_id'=>$company_check->id,'status'=>1);
               evaluation_element::create($input);
            }
         }
    ...
    //return $jobs_evaluations;
      ...
      return view('jobs.job-list',compact('evaluation','jobs','evaluation_keies','subdomain'));
    }
    ...
    //return response($request);
      $validator = Validator::make($request->all(),
        ...
        if ($validator->passes())
        {
          ...
          $input=array('title'=>$evalelem['title'] ,'target_location_check'=>$evalelem['target_location_check'],'company_id'=> $company->id,'client_location_check'=>$evalelem['client_location_check'],'outdoor_without_attend'=>$evalelem['outdoor_without_attend']);

          $job=job::create($input);
    
         //get last id add in job table for add in element_evalution_table
          $job_id=job::latest()->first('id');
     
        //get not empty degree only
      

     if(!empty($evalelem['degree']))
    {
    ...
    $evaluationelements= evaluation_element::where('company_id', $company_check->
            id)->get();
        return response()->json(['jobevalution' => $data, 'jobs' => $jobeval,'evaluation_elements'=>$evaluationelements]);
...
if ($req->passes())
        {
          ...
          //update in jobstable
          Job::where('id',$evalelem2['jobEval_id'])->where('company_id', $company_check->
          id)->update(['title'=>$evalelem2['title'],'target_location_check'=>$evalelem2['target_location_check'],'client_location_check'=>$evalelem2['client_location_check'],'outdoor_without_attend'=>$evalelem2['outdoor_without_attend']]);

          evaluation_elements_jobs::where('job_id', $id)->where('company_id', $company_check->
          id)->delete();
      
        if(isset($evalelem2['degree']))
           {
    ...
    public function showelementevaluation($subdomain)
    {
         $company_check = $this->company;
           
            $data= evaluation_element::where('company_id', $company_check->
                id)->get();
                
           //return response('$evaluationelements'=>$evaluationelements);   
           return response()->json(['evaluationelements' => $data]);
